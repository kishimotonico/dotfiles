...
codex exec --json -s workspace-write -c approval_policy=never "<指示>" > "$out" 2>"$err" </dev/null
```
...
- approval policy は `never` にする。委譲先のCodexが承認プロンプトで止まっても誰も応答できない。`never` なら sandbox で実行できない操作は失敗としてモデルに即返り、Codexが代替手段を試すか報告してくる。**`codex exec` に `-a` フラグは無い**(codex-cli 0.144.5、付けると usage エラー)ので `-c approval_policy=never` で指定する。対話TUIの `codex` には `-a never` が使える
...
修正を差し戻すときは、手順3で取った thread_id を明示して resume する。**`resume` サブコマンドに `-s` / `-a` は存在しない**(codex-cli 0.144.5 で確認。付けると `unexpected argument` で即エラー)。sandbox と approval は `-c` で指定する:

```bash
codex exec resume <thread_id> --json -c 'sandbox_mode="workspace-write"' -c 'approval_policy="never"' "<修正指示>" > "$out2" 2>"$err2" </dev/null
```
...
self=$(herdr pane list | jq -r --arg cwd "$PWD" '[.result.panes[] | select(.agent=="claude" and .cwd==$cwd)][0].pane_id')
pane=$(herdr pane split "$self" --direction down --no-focus | jq -r '.result.pane.pane_id')
herdr pane run "$pane" "codex exec -s workspace-write -c approval_policy=never '<指示>' 2>&1 | tee $out"'; echo "CODEX""_DONE:$?"'
