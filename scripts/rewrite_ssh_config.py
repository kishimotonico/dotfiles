...
def command_execute(command: str) -> Optional[str]:
    ...
    """
    try:
        # 実行中の様子がわかるように少しリッチにする場合はここにスピナーなどを入れることも可能
...
def select_tasks(tasks: List[UpdateTask]) -> List[UpdateTask]:
    ...
    print("\n[選択] 更新したいディレクティブを選んでください (Tabで複数選択可 / Enterで決定)")
    ...
    try:
        selected_names = FuzzyPrompt(
            ...
            instruction="[Tab] 選択/解除, [Enter] 確定, [Ctrl+C] キャンセル",
            validate=lambda result: len(result) > 0,  # 1つ以上選択必須にする場合
            invalid_message="少なくとも1つ選択してください",
        ).execute()
    except KeyboardInterrupt:
        # Ctrl+C が押された場合は空リストを返してキャンセル扱いにする
        return []

    # 選択された名前に紐づく全タスクを結合して返す
    selected_tasks = []
    for name in selected_names:
        selected_tasks.extend(tasks_by_name[name])
